...
class AddressBook(UserList):
    ...
    def search(self, pattern, category):
        ...
        for account in self.data:
            if category_new == 'phones':
                ...
                result.append(account)
        if not result:
            PresentErrors().present('There is no such contact in address book!')
        return result
    ...
    def edit(self, contact_name, parameter, new_value):
        ...
        try:
            ...
            PresentErrors().present('Incorrect parameter! Please provide correct parameter')
        except NameError:
            PresentErrors().present('There is no such contact in address book!')
        else:
